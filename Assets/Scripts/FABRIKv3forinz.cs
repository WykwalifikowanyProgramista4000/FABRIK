...
public class FABRIKv3forinz : MonoBehaviour
{
    ...
    void Update()
    {
        if (_sillyLittleFabrik)
        {
            SillyLittleFabrik();
        }
    }

    private void SillyLittleFabrik()
    {
        Forwards();
        Backwards();
    }
    ...
    public void Forwards()
    {
        ...
        {
            target = m_RestPoint.transform.position;
        }
        else
        {
            target = m_TargetPoint.transform.position;
        }

        if (Vector3.Distance(target, m_Joints[m_Joints.Count - 1].transform.position) <= m_DistanceFromTargetTolerance) { return; }
